...
tokens {
    BEGIN_BLOCK, // Explicit token for block start (replaces INDENT)
    END_BLOCK,   // Explicit token for block end (replaces DEDENT)
    END_STATEMENT // Explicit token for statement end (replaces NEWLINE role)
    // INDENT,    // Removed
    // DEDENT     // Removed
}
...
COLOR:   COLOR_LITERAL;
NEWLINE: OS_INDEPENDENT_NL -> channel(HIDDEN); // Moved to HIDDEN channel

// EXPLICIT TOKENS (Expected to be generated by preprocessor)
// Using unlikely character sequences to avoid clashes with user code.
// The preprocessor will generate these exact strings.

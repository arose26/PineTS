...
start_expression: expression END_STATEMENT? EOF;
...
statement
    : compound_statement_with_block END_BLOCK* // Block statements consume optional trailing END_BLOCKs
    | simple_statement END_STATEMENT           // Simple statements require END_STATEMENT
    | compound_assignment END_STATEMENT        // Compound assignments require END_STATEMENT
    ;

// Define compound statements that inherently contain blocks
compound_statement_with_block
...
// Allow RHS to be regular expression OR structure expression
compound_name_initialization:  var_declaration_stmt EQUAL (expression | structure_expression);
compound_tuple_initialization: tuple_declaration EQUAL (expression | structure_expression);
compound_reassignment:  assignment_target COLONEQUAL (expression | structure_expression);
compound_augassignment: assignment_target augassign_op (expression | structure_expression);
...
type_declaration: EXPORT? TYPE name BEGIN_BLOCK field_definitions END_BLOCK;

field_definitions: field_definition+;
field_definition:  type_specification name_store (EQUAL expression)? END_STATEMENT;
...
switch_structure: SWITCH expression? BEGIN_BLOCK switch_cases END_BLOCK;
...
local_block: BEGIN_BLOCK statements END_BLOCK ;

// SIMPLE ASSIGNMENTS

/* Removed simple_assignment rules from here, they feed into compound_assignment
simple_assignment
...
var_declaration_stmt: declaration_mode? type_specification? name_store;
